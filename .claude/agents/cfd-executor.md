...
**一次真实运行的教训**：5 万单元、单核、3000 步 = **45 分钟**，而前 100 步就已经
能看出它不会收敛（残差停在 4.4e-2 的平台、出口回流持续存在）。
**那 45 分钟里约 40 分钟是可以省下的。**

**流程**：

```
1. 先跑 50-100 步（几分钟），记录：
     · 每步耗时            → 据此估算总时长
     · 残差是在降、在平、还是升
     · Fluent 有没有反复报同一条警告（尤其 "Reversed flow"）
2. 按下面的判据决定继续还是停
3. 停下来时【如实汇报】，不要静默放弃
```
...
| 信号 | 判据 |
|---|---|
| **残差平台** | 100 步内残差**没有下降一个数量级**，且趋势是平的 |
| **目标不相称** | 规范定的残差目标比**当前平台**低 2 个数量级以上 |
| **持续回流** | `Reversed flow on N faces` 反复出现，且**不是只在启动阶段** |
| **耗时超预期** | 按实测每步耗时估算，跑完要远超合理范围 |

**汇报里要写清**：跑到第几步、残差停在多少、目标是多少、观测到的警告、
以及你的判断（是规范目标不可达，还是执行有问题）。**那是审查环节需要的输入。**

> **不要把这些当成"我自己判了就不跑了"。** 停下来是**把判断权交出去**——
> 交给审查 agent 和用户，而不是替他们决定。

### 规范若给了不相称的目标，照做但要点出来

规范给残差目标 `1e-6` 又只给 `max_iterations=3000` 时，**数学上就是必然跑满**。
这种情况**照做，但在汇报里明确指出** —— 那是规范侧的问题，归 `spec` 路由。

### ★ 把判据设进 Fluent，让它自己停 —— 别用固定步数封顶

**实测确认**：`iterate(iter_count=N)` 里的 N 是**上限不是任务量**。
Fluent 每步都检查 `check_convergence`，残差达到 `absolute_criteria` 就打印
`!   N solution is converged` 并停下。实测给 500 步额度、判据设 1e-2，
**第 21 步就停了**。
...
for eq in list(r.equations.get_state()):
    r.equations[eq].absolute_criteria = <规范里的目标>
solver.settings.solution.run_calculation.iterate(iter_count=<规范里的上限>)
```

**所以要检查一件事**：规范给的目标**够不够得到**？

- 够得到 → Fluent 自己停，`iter_count` 只是保险丝
- **够不到 → 提前停永不触发，`iter_count` 变成必然跑满的步数**

**判断依据**：先跑几十步看残差趋势。若已在平台、且平台比规范目标高 2 个数量级以上，
**目标就是够不到的** —— 这时按上面「先探 100 步」的做法停下来汇报。

详见 `references/pyfluent-recipes.md` 的「`iterate` 会自己提前停」。

### 分批跑，并盯住时间

开场先跑一小批**实测每步耗时**，据此估算总时长。若估算远超预期，
不要闷头跑完 —— 停下来汇报。
...
### ★ 规范修订后，把作废的裁决挪进 `_superseded_v<N>/`

**这是"不重跑"的代价，必须由你处理。**

规范升到 v2 后，`04_results/` 里那些**按 v1 判据算出来的裁决**仍然躺在原地。
它们不会自己更新，而且往往在 `scope` 字段里**声称**是按"当前 `02_spec.json`"评的 ——
**规范一改，那句话就变成假的**。

**而 `04_results/` 是审查者的输入目录。** 后来的人翻它时，会看到
`summary.json` 说 C3 FAIL、而 `review_N.json` 说全通过 —— 互相矛盾。

```
04_results/
├── _superseded_v1/          ← 按 v1 口径算的【裁决】挪进来
...
| 该挪 | 留下 |
|---|---|
| 按旧判据口径算出的**裁决/评估** | 求解产物（`.cas.h5` / `.dat.h5` / 残差 / 监测历史） |
| `spec_revision` 或 `scope` 指向旧版的汇总 | **与判据无关的原始测量**（几何、通量、剖面 CSV） |

> **原始测量不该挪** —— 它们和判据版本无关。v2 换的是判据，数据还是那些数据。
> **挪的是解释，不是事实。**

**并在汇报里说明挪了哪些**，让审查者知道。

### ★ 文件名是 `.json`，内容就必须是合法 JSON

**实测发生过**：一个 `summary.json` 里写着

```python
"residual_targets": {e: 1e-06 for e in ["continuity", "x-velocity", ...]},
```

**那是 Python 的字典推导式，不是 JSON。** 后果：任何 `json.load` 都直接抛异常，
而那个文件**正是审查者要读的证据**。而且它既不是合法 JSON、也不是合法 Python
字面量（`ast.literal_eval` 同样失败）—— 两边都用不了。

**写盘前自检**：

```bash
.venv/Scripts/python.exe -c "import json,sys; json.load(open(sys.argv[1],encoding='utf-8')); print('ok')" <文件>
```

**注意你不是在写 Python 文件**：`{k: v for k in ...}` 这类推导式、
`True`/`False`/`None`（应为 `true`/`false`/`null`）、单引号字符串，
在 `.json` 里都非法。
