...
### ★★ 但「不重跑」会留下一批被作废的裁决 —— 必须隔离

**这是"不重跑"的代价，必须显式处理。**

规范修订后，`04_results/` 里那些**按旧版判据算出来的裁决**仍然躺在原地：

- 它们是**按旧规范口径**算的（例如 v1 下 C8 与 C3 绑定，C3 一挂 C8 就被判
  `NOT EVALUABLE`；v2 已解绑，但文件不会自己改）
- 它们的 `scope` 字段往往**声称**自己是按"当前 `02_spec.json`"评的 ——
  规范一改，**那句话就变成假的**
- **而 `04_results/` 是审查者的输入目录**

**后果**：后来的人（或 agent）翻证据目录时，会看到互相矛盾的裁决 ——
`summary.json` 说 C3 FAIL，而 `review_N.json` 说全通过。**报告里写了提示也不够**，
因为不是每个人都会先读报告。

**规则**：规范修订导致某批裁决作废时，**把作废的文件移进 `_superseded_v<N>/`**，
而不是留在原地。

```
04_results/
├── _superseded_v1/          ← 按 v1 口径算的裁决全挪进来
│   ├── criteria_assessment.json
│   ├── summary.json
│   └── dp_comparison.json
├── ...                      ← 仍然有效的证据留在外层
└── summary.json             ← 若重新生成过，这里放新版
```

**判据（哪些该挪）**：

| 该挪进 `_superseded_v<N>/` | 该留在原地 |
|---|---|
| 按旧判据口径算出的**裁决/评估** | 求解产物（`.cas.h5` / `.dat.h5` / 残差 / 监测历史）|
| `scope` 或 `spec_revision` 指向旧版的汇总文件 | **与判据无关的原始测量**（几何、通量、剖面 CSV）|

> **原始测量不该挪** —— 它们和判据版本无关，v2 只是换了判据，数据还是那些数据。
> 挪的是**解释**，不是**事实**。

**并且在 `04_results/README.md` 或报告里写明**：`_superseded_v<N>/` 里是按第 N 版
规范算的，**不要当作当前结论**。
